...
bp_animales = Blueprint("bp_animales", __name__)

# ==================== RUTAS DE ANIMALES ====================

@bp_animales.route("/animales", methods=["GET"])
def listar_animales():
    return jsonify([animal.toDiccionario() for animal in repo_animales.obtenerAnimales()])


@bp_animales.route("/animales/<string:nombre>", methods=["GET"])
...
@bp_animales.route("/animales", methods=["POST"])
def agregar_animal():
    datos = request.json
    try:
        ...
        return jsonify({
            "Mensaje": f"{tipo.capitalize()} agregado con éxito.",
            "animal": animal.toDiccionario()
        }), 201
...
@bp_animales.route("/animales/<string:nombre>", methods=["PUT"])
def modificar_animal(nombre):
    ...
    try:
        ...
        if repo_animales.actualizarAnimal(nombre, animal):
            return jsonify({
                "Mensaje": f"{tipo.capitalize()} actualizado con éxito.",
                "animal": animal.toDiccionario()
            })
...
@bp_animales.route("/animales/<string:nombre>", methods=["DELETE"])
def eliminar_animal(nombre):
    if repo_animales.eliminarAnimal(nombre):
        return jsonify({"Mensaje": "Animal eliminado con éxito."})

    return jsonify({"error": "No se encontró el animal a eliminar"}), 404


@bp_animales.route("/animales/<string:nombre>/precio", methods=["GET"])
def obtener_precio_animal(nombre):
    ...
    return jsonify({
        "nombre": animal.obtenerNombre(),
        "precio_adopcion": animal.obtenerPrecio()
    })
